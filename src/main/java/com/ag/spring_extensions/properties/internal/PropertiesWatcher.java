...
/**
 * This class creates the Threads that watch for changes in the properties files.
 * When a property changes, the corresponding Thread uses the EventPublisher
 * to alert objects interested in the update.
 */
public class PropertiesWatcher implements Runnable {
	...
	public PropertiesWatcher(final Resource[] locations, final EventPublisher eventPublisher)
        throws IOException
    {
	...
	private class ResourceWatcher implements Runnable {
		...
		public void run() {
			try {
				log.debug("Start watching for modification events for path {}", this.path.toString());
				while (!Thread.currentThread().isInterrupted()) {
					...
					} catch (final ClosedWatchServiceException | InterruptedException ex) {
                        String msg = String.format("Stop watching for modification events for path %s due to watch service related exception %s",
                                this.path.toString(), ex.getMessage());
                        log.debug(msg);
					...
					if (watchKey != null) {
						...
						if (!watchKey.reset()) {
                            String msg = String.format("Stop watching for modification events for path %s. WatchKey not reset.",
                                    this.path.toString());
                            log.debug(msg);
							Thread.currentThread().interrupt();
							return;
        ...
        {
			log.debug("Watched Resource changed, modified file [{}]", target.getFileName().toString());
