...
import com.ag.spring_extensions.properties.bean.BeanPropertyHolder;
import com.ag.spring_extensions.properties.bean.PropertyModifiedEvent;
import com.ag.spring_extensions.properties.conversion.PropertyConversionService;
import com.ag.spring_extensions.properties.event.PropertyChangedEventNotifier;

/**
 * <p>
 * Processes beans on start-up, injecting field values marked with {@link ReloadableProperty}
 * setting the associated annotated property value with properties
 ...
 * configured {@link ReadablePropertySourcesPlaceholderConfigurer} which are changed.
 * Once a property is reloaded the associated bean holding that value will have its
 * property updated, no further bean operations are performed on the reloaded bean.
 * </p>
 * <p>
...
public class ReloadablePropertyPostProcessor extends InstantiationAwareBeanPostProcessorAdapter {
	...
	/**
	 * Utility method to register the class for receiving events about property file updates,
    ...
    /**
     * Utility method to un-register the class from receiving events about property files being changed.
     */
	...
	/**
	 * Method subscribing to {@link PropertyModifiedEvent} through Guava's {@link Subscribe} annotation.
     * When invoked, this method updates fields annotated with the {@link ReloadableProperty} annotation
	...
	public void updateField(final BeanPropertyHolder holder, final PropertyModifiedEvent event)
    {
		...
		try {
			log.info("Reloading property [{}] on field [{}] for class [{}]",
                    new Object[] { event.getPropertyName(), fieldToUpdate.getName(), canonicalName });
	...
	private void validatePropertyAvailableOrDefaultSet(final Object bean, final Field field,
                                                       final ReloadableProperty annotation,
	...
	private boolean fieldDoesNotHaveDefault(final Field field, final Object value)
        throws IllegalArgumentException, IllegalAccessException
    {
		try {
